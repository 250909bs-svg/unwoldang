...
/**
 * 패널 14 명식 요약. 리포트 CH01 의 네 칸 요약(c01-4)과 1:1 로 대응한다.
 * 값은 전부 마스킹이고 라벨·구조만 실물과 같다. 없는 항목을 만들어 붙이지 않는다.
 *
 * 리포트 CH01 에는 이 네 줄 **앞에** 시·일·월·년 4기둥 원국 패널(c01-3)이 한 장 더 있다.
 * 상세페이지에 그 표까지 옮기면 같은 표가 두 번 나오므로 여기서는 요약 네 줄만 보여주고,
 * 문구에서 "리포트가 내는 건 이 네 줄이 전부"라고 **단정하지 않는다** —
 * 실물보다 적게 약속하는 것은 괜찮지만, 없다고 말해 두고 내보내면 표시 문제가 된다.
 */
...
export const reunionCuts: readonly ReunionCut[] = [
  ...
  },
  {
    ...
    note: '운월당은 이 신호들을 사실로 올려 두지 않습니다. 리포트는 알려주신 것(◇)과 계산한 것(◆)을 표시로 갈라 적습니다.'
  },
  {
    n: '05',
    act: 1,
