...
const CUSTOMER_ADDITIONAL_FORBIDDEN_PATTERNS: ReadonlyArray<[RegExp, string]> = [
  [/겁재은|식신와|정인가|승부을|편관와|정관가|정인와|비견가|(?:무|기)은 (?:편인|정인)|사은 사건|태은 사건|무은지형|돈·사업 구조과|비교적 균형적인 편입니다로 판정했습니다|관계의 역할 배치을|판단 순서으로|대운 진입 전 대운|고려하면로 보이므로|관점 관점|(?:약한|강한) 편입니다 통근|일간 (?:(?:무|기|계)은|(?:갑|을|병|정|경|신|임)는)|은\(는\)/, '잘못된 한국어 조사'],
