...
type FocusedReportKind = Exclude<SajuReportData['kind'], 'comprehensive'>;
type ReportCardTone = NonNullable<ReportSection['cards']>[number]['tone'];

const FOCUSED_REPORT_META: Record<
  FocusedReportKind,
  {
    title: string;
    subtitle: string;
...
> = {
  yearly: {
    title: '인생흐름 대운 리포트',
    subtitle: '대운, 세운, 월운을 시간표처럼 정리해 앞으로 어느 시기에 무엇을 잡고 무엇을 줄여야 하는지 보여드립니다.',
    badge: 'LIFE FLOW',
    coverLabel: '인생흐름 리포트'
  },
  love: {
    title: '연애운 리포트',
    subtitle: '내 연애 패턴, 들어오는 인연상, 연락과 표현의 타이밍을 현실적인 관계 장면으로 정리합니다.',
    badge: 'LOVE READING',
    coverLabel: '연애운 리포트'
  },
  reunion: {
    title: '재회 가능성 리포트',
    subtitle: '남아 있는 감정선과 다시 연락할 타이밍, 멈춰야 할 신호를 차분하게 구분합니다.',
    badge: 'REUNION',
    coverLabel: '재회운 리포트'
  },
  marriage: {
    title: '결혼운 리포트',
    subtitle: '배우자상, 혼인 적기, 결혼 후 생활 리듬과 현실 조건을 함께 보는 결혼 설계 리포트입니다.',
    badge: 'MARRIAGE',
    coverLabel: '결혼운 리포트'
  },
  compatibility: {
    title: '궁합 리포트',
    subtitle: '내 관계 기질을 기준으로 잘 맞는 상대 리듬, 갈등 지점, 오래 가는 관계 조건을 정리합니다.',
    badge: 'COMPATIBILITY',
    coverLabel: '궁합 리포트'
  }
};

function productCard(title: string, body: string, tone?: ReportCardTone, badge?: string) {
  return { title, body, tone, badge };
}
...
function retitleSection(
  ...
  const base = getSection(report, id);
...
function section(id: string, title: string, subtitle: string, patch: Partial<ReportSection> = {}): ReportSection {
  return {
    id,
    title,
    subtitle,
...
function bestMonthLabel(report: SajuReportData) {
  const best = [...report.monthLuck].sort((left, right) => right.score - left.score)[0];
  if (!best) return '가장 흐름이 열리는 달';

  return `${best.year}.${String(best.month).padStart(2, '0')} ${getLuckPhase(best.score)}`;
}

function watchMonthLabel(report: SajuReportData) {
  const watch = [...report.monthLuck].sort((left, right) => left.score - right.score)[0];
  if (!watch) return '속도를 줄여야 하는 달';

  return `${watch.year}.${String(watch.month).padStart(2, '0')} ${getLuckPhase(watch.score)}`;
}

function buildYearlyFocusedSections(report: SajuReportData): ReportSection[] {
  const currentYear = report.yearLuck[0];

  return [
    section('life-flow-core', '인생흐름 핵심 판정', '지금 삶이 어느 계절에 들어와 있는지 먼저 짚습니다.', {
      callout: {
        title: `${report.currentDayun.name} 대운의 중심`,
        body: `${report.currentDayun.range} 구간은 ${report.currentDayun.focus} 쪽으로 흐름이 모입니다. 좋은 시기를 기다리는 리포트가 아니라, 언제 힘을 쓰고 언제 정비해야 하는지 구분하는 시간표로 보시면 좋습니다.`
      },
      cards: [
        productCard('현재 대운', `${report.currentDayun.summary} 다만 ${report.currentDayun.caution}`, 'good'),
        productCard('다음 대운', `${report.nextDayun.name} 구간은 ${report.nextDayun.focus} 흐름이 커집니다. 지금부터 준비하면 전환 시기의 부담이 줄어듭니다.`),
        productCard('올해 초점', currentYear ? `${currentYear.year}년은 ${currentYear.headline}. 핵심은 ${currentYear.focus}입니다.` : report.heroNote),
        productCard('주의 구간', `${watchMonthLabel(report)}에는 결정 속도를 늦추고 관계, 돈, 체력을 다시 맞추는 쪽이 안정적입니다.`, 'warn')
      ]
    }),
    retitleSection(report, 'fortune', '120세 대운 타임라인', '10년 단위 운의 도로 상태를 현재와 다음 구간 중심으로 봅니다.'),
    retitleSection(report, 'year', '향후 5년 사건 타임라인', '언제 무엇이 왜 움직이는지 연도별 핵심 사건성으로 정리합니다.'),
    retitleSection(report, 'month', '가까운 12개월 실행 캘린더', '점수보다 확장기, 정비기, 회복기처럼 실제 행동 단계로 읽습니다.'),
    retitleSection(report, 'detail12', '12운성 에너지 리듬', '대운마다 힘이 생기고 줄어드는 생장 리듬을 확인합니다.'),
    retitleSection(report, 'detailRel', '합·충·형·파 전환 신호', '좋은 운도 원국과 부딪히면 조정이 필요하고, 평범한 운도 합이 되면 기회가 됩니다.'),
    retitleSection(report, 'detailSal', '귀인·신살 활용 포인트', '도움이 들어오는 사람, 이동성, 주목받는 방식처럼 보조 신호를 현실적으로 풉니다.'),
    retitleSection(report, 'element', '오행 체감 밸런스', '단순 비율보다 월령, 통근, 대운에서 다시 살아나는 기운까지 함께 봅니다.')
  ];
}

function buildLoveFocusedSections(report: SajuReportData): ReportSection[] {
  return [
    section('love-core', '현재 연애 기류', '지금 마음이 어디서 열리고 어디서 닫히는지 먼저 봅니다.', {
      callout: {
        title: `${report.customerName}님의 관계 온도`,
        body: `연애운은 단순히 사람이 들어온다는 말보다, 내가 어떤 속도의 사람에게 안정감을 느끼는지 보는 쪽이 정확합니다. ${bestMonthLabel(report)}에는 만남이나 대화의 문이 비교적 열리고, ${watchMonthLabel(report)}에는 애매한 말보다 분명한 확인이 필요합니다.`
      },
      cards: [
        productCard('끌리는 사람', '말이 화려한 사람보다 생활 리듬, 약속 태도, 책임 범위가 안정적인 사람에게 마음이 오래 갑니다.', 'good'),
        productCard('마음이 닫히는 순간', '상대가 애매하게 굴거나 연락의 기준이 흔들리면 호감보다 피로감이 먼저 올라올 수 있습니다.', 'warn'),
        productCard('연애의 시작점', '소개, 반복적으로 마주치는 생활권, 일이나 취미가 겹치는 공간처럼 자연스럽게 쌓이는 인연이 유리합니다.'),
        productCard('이번 리포트의 초점', '상대 맞추기보다 내 감정 속도, 표현 방식, 관계 기준을 먼저 정리하는 데 있습니다.')
      ]
    }),
    retitleSection(report, 'love', '내 연애 원국 패턴', '어떤 방식으로 마음을 열고 확신을 얻는지 사주 원국 기준으로 봅니다.'),
    section('love-person', '들어오는 인연상', '오래 갈 가능성이 큰 상대의 분위기와 피해야 할 상대 유형을 나눕니다.', {
      cards: [
        productCard('잘 맞는 분위기', '감정 기복을 과시하지 않고, 일상에서 약속을 지키며, 관계를 서두르지 않는 사람이 잘 맞습니다.', 'good'),
        productCard('주의할 분위기', '말은 뜨겁지만 일정, 돈, 책임에 대한 태도가 흐린 사람은 초반 설렘 뒤 피로를 크게 만들 수 있습니다.', 'warn'),
        productCard('만남이 열리는 장소', '지인 소개, 업무 동선, 동네 생활권, 배움의 공간처럼 반복 접점이 생기는 곳이 좋습니다.'),
        productCard('확신의 기준', '고백의 강도보다 서로의 생활에 무리가 없는지, 갈등 뒤 다시 맞출 태도가 있는지가 중요합니다.')
      ]
    }),
    section('love-tempo', '연락·표현 타이밍', '연락 빈도보다 안정적으로 이어지는 대화 리듬이 핵심입니다.', {
      details: [
        {
          summary: '연락을 늘려도 좋은 때',
          content: `${bestMonthLabel(report)}처럼 흐름이 열린 구간에는 짧고 분명한 제안이 좋습니다. 길게 설명하기보다 언제, 어디서, 무엇을 할지 보이는 말이 관계를 앞으로 움직입니다.`,
          open: true
        },
        {
          summary: '잠깐 멈춰야 하는 때',
          content: `${watchMonthLabel(report)}에는 상대 반응을 확인하려고 말을 늘리기보다 하루 정도 간격을 두고 내 감정이 정말 같은지 보는 편이 좋습니다. 불안한 상태에서 보낸 긴 문장은 관계를 무겁게 만들 수 있습니다.`,
          open: true
        }
      ]
    }),
    retitleSection(report, 'year', '연애 이벤트 타임라인', '향후 흐름에서 만남, 정리, 재정의가 일어나기 쉬운 시기를 봅니다.'),
    retitleSection(report, 'month', '30일 관계 캘린더', '바로 움직일 달과 지켜봐야 할 달을 구분합니다.'),
    section('love-caution', '피해야 할 관계 신호', '좋아하는 마음과 오래 갈 수 있는 관계를 구분합니다.', {
      bullets: [
        '상대의 말보다 일정과 행동이 계속 어긋나는 관계',
        '연락이 올 때만 마음이 풀리고, 만나고 나면 다시 불안해지는 관계',
        '관계 정의를 계속 미루면서 감정 소모만 커지는 흐름',
        '내 생활 리듬, 일, 돈의 기준을 무너뜨려야 유지되는 관계'
      ]
    })
  ];
}

function buildReunionFocusedSections(report: SajuReportData): ReportSection[] {
  return [
    section('reunion-core', '재회 가능성 핵심 판정', '미련과 가능성을 분리해서 봅니다.', {
      callout: {
        title: '재회의 첫 기준',
        body: `재회운은 상대 마음을 확정하는 풀이가 아니라, 다시 연결했을 때 같은 문제가 반복될지 줄어들지를 보는 리포트입니다. ${bestMonthLabel(report)}에는 부드러운 확인이 가능하고, ${watchMonthLabel(report)}에는 감정적 메시지를 피하는 편이 좋습니다.`
      },
      cards: [
        productCard('가능성이 남는 경우', '헤어진 이유가 감정 소진보다 상황, 일정, 오해에 가까웠다면 천천히 복구할 여지가 있습니다.', 'good'),
        productCard('반복 위험이 큰 경우', '같은 말다툼, 같은 회피, 같은 불신이 아직 해결되지 않았다면 재회보다 원인 정리가 먼저입니다.', 'warn'),
        productCard('연락의 톤', '미안함을 길게 설명하기보다 짧게 안부를 묻고, 상대가 답할 여지를 남기는 방식이 좋습니다.'),
        productCard('최종 판단', '상대 반응 하나로 결론을 내리지 말고, 2주 안에 행동이 안정적으로 반복되는지 보세요.')
      ]
    }),
    section('reunion-feeling', '남아 있는 감정선', '감정의 깊이보다 다시 쌓을 수 있는 신뢰의 여지를 봅니다.', {
      paragraphs: [
        '재회에서 중요한 것은 아직 좋아하느냐보다 다시 만나도 같은 상처를 반복하지 않을 수 있느냐입니다.',
        `${report.dayMaster} 일간의 흐름에서는 말보다 행동 기준이 분명해야 마음이 편해집니다. 그래서 재회도 감정 고백보다 달라진 생활 태도가 먼저 보여야 합니다.`
      ],
      cards: [
        productCard('남은 미련', '상대의 좋은 기억만 붙잡는 미련인지, 실제로 다시 맞출 조건이 남아 있는지 구분해야 합니다.'),
        productCard('복구 포인트', '연락 빈도, 갈등 후 태도, 약속 방식 중 하나라도 이전보다 안정되어야 재회가 오래 갑니다.', 'good')
      ]
    }),
    retitleSection(report, 'month', '재접촉 타이밍 캘린더', '연락하기 좋은 구간과 멈춰야 할 구간을 나눕니다.'),
    retitleSection(report, 'year', '관계 재편 타임라인', '다시 이어질지, 정리될지, 새 인연으로 넘어갈지의 흐름을 봅니다.'),
    section('reunion-message', '연락 금지·허용 문장', '감정이 앞설 때 관계를 망치지 않는 문장 기준입니다.', {
      details: [
        {
          summary: '보내도 되는 문장',
          content: '“잘 지내는지 궁금해서 짧게 연락했어. 답이 부담스러우면 천천히 해도 괜찮아.”처럼 상대의 선택권을 남기는 문장이 좋습니다.',
          open: true
        },
        {
          summary: '피해야 할 문장',
          content: '“왜 답 안 해?”, “네가 먼저 그랬잖아”, “마지막으로 한 번만 보자”처럼 압박과 죄책감을 건드리는 문장은 재회 가능성을 낮춥니다.',
          open: true
        }
      ]
    }),
    section('reunion-plan', '7일·30일 복구 플랜', '재회는 한 번의 연락보다 회복 리듬이 중요합니다.', {
      bullets: [
        '1~3일: 감정이 올라오는 이유를 적고 바로 연락하지 않습니다.',
        '4~7일: 짧은 안부나 사과가 필요한지 문장을 정리합니다.',
        '2주차: 상대 반응이 있으면 속도를 늦추고 일상 대화부터 회복합니다.',
        '30일 안에 같은 문제를 다시 확인하면 재회보다 정리를 우선합니다.'
      ]
    })
  ];
}

function buildCompatibilityFocusedSections(report: SajuReportData): ReportSection[] {
  return [
    section('compat-core', '궁합 핵심 판정', '두 사람 비교 전, 내 관계 기질을 먼저 정확히 잡습니다.', {
      callout: {
        title: '궁합의 기준',
        body: '궁합은 단순히 잘 맞는다, 안 맞는다가 아니라 서로의 생활 리듬과 갈등 복구 방식이 맞물리는지 보는 것입니다. 상대 정보가 추가되면 이 기준 위에 상대 원국을 겹쳐 더 정밀하게 볼 수 있습니다.'
      },
      cards: [
        productCard('잘 맞는 상대', '약속과 생활 기준이 분명하고, 감정 문제를 대화로 복구하려는 사람이 좋습니다.', 'good'),
        productCard('부딪히는 상대', '즉흥성이 강하고 책임 범위가 흐린 사람과는 초반 재미 뒤 피로가 커질 수 있습니다.', 'warn'),
        productCard('오래 가는 조건', '연락 빈도보다 갈등 후 다시 맞추는 태도, 돈과 시간의 기준, 서로의 독립성이 중요합니다.'),
        productCard('비교 포인트', '상대의 일간, 월지, 일지, 대운을 함께 보면 끌림과 충돌 지점을 더 정확히 볼 수 있습니다.')
      ]
    }),
    retitleSection(report, 'love', '내 관계 원국 패턴', '상대와 맞춰보기 전, 내가 관계에서 반복하는 기준을 봅니다.'),
    retitleSection(report, 'detailRel', '합·충·형·파 관계 신호', '관계에서 붙는 지점과 부딪히는 지점을 명리 구조로 확인합니다.'),
    retitleSection(report, 'element', '생활 궁합 오행 밸런스', '서로의 생활 속도, 감정 온도, 현실 감각이 어디서 맞물리는지 봅니다.'),
    retitleSection(report, 'ten', '십성으로 보는 관계 역할', '내가 관계에서 챙기는 역할, 기대하는 역할, 피로해지는 역할을 구분합니다.'),
    section('compat-conflict', '갈등 패턴과 회복 방식', '좋은 궁합도 갈등 복구 방식이 맞지 않으면 오래 가기 어렵습니다.', {
      details: [
        {
          summary: '잘 맞을 때의 모습',
          content: '서로의 시간을 존중하면서도 필요한 순간에는 분명히 곁에 있어주는 관계가 좋습니다. 일상 루틴과 약속 기준이 맞으면 애정 표현이 크지 않아도 안정감이 쌓입니다.',
          open: true
        },
        {
          summary: '틀어질 때의 모습',
          content: '한쪽은 확인을 원하고 한쪽은 시간을 벌려고 하면 관계가 급격히 지칩니다. 이때는 감정 해석보다 약속, 일정, 역할을 다시 정리해야 합니다.',
          open: true
        }
      ]
    }),
    retitleSection(report, 'year', '관계 변화 타임라인', '가까워지는 시기, 조정이 필요한 시기, 거리 조절이 필요한 시기를 봅니다.')
  ];
}

function buildMarriageFocusedSections(report: SajuReportData): ReportSection[] {
  return [
    section('marriage-core', '결혼운 핵심 판정', '결혼 가능성보다 결혼이 안정적으로 유지되는 조건을 먼저 봅니다.', {
      callout: {
        title: '결혼운의 핵심',
        body: `결혼은 인연운만으로 보지 않습니다. 배우자상, 생활 리듬, 돈의 기준, 가족과 역할 분담이 함께 맞아야 안정됩니다. ${bestMonthLabel(report)}에는 관계를 현실 주제로 꺼내기 좋고, ${watchMonthLabel(report)}에는 결론을 서두르지 않는 편이 좋습니다.`
      },
      cards: [
        productCard('배우자상', '성실하고 생활 기준이 분명하며, 관계를 말보다 행동으로 안정시키는 사람이 잘 맞습니다.', 'good'),
        productCard('혼인 적기', '감정이 뜨거운 시기보다 돈, 일정, 가족, 주거 이야기를 현실적으로 맞출 수 있는 시기가 더 중요합니다.'),
        productCard('주의할 결혼 흐름', '상대의 조건을 내가 혼자 감당하거나, 불안을 줄이려고 결혼을 서두르는 흐름은 피해야 합니다.', 'warn'),
        productCard('확인 기준', '갈등이 생겼을 때 서로를 설득하려 하기보다 생활 조건을 다시 협의할 수 있는지가 핵심입니다.')
      ]
    }),
    retitleSection(report, 'love', '배우자상과 관계 기질', '연애 감정이 결혼 생활로 이어질 때 어떤 상대가 안정적인지 봅니다.'),
    retitleSection(report, 'year', '혼인 적기 타임라인', '결혼 논의, 상견례, 동거, 주거 결정처럼 현실 이벤트가 움직이기 쉬운 때를 봅니다.'),
    retitleSection(report, 'month', '결혼 준비 캘린더', '바로 결정할 달과 예산·가족·생활 조건을 점검할 달을 나눕니다.'),
    retitleSection(report, 'money', '결혼 후 돈의 기준', '수입보다 지출 방식, 공동비, 저축 규칙이 안정감을 좌우합니다.'),
    retitleSection(report, 'health', '생활 리듬과 체력 궁합', '결혼 후 무너질 수 있는 수면, 일, 감정 회복 리듬을 미리 봅니다.'),
    retitleSection(report, 'detailRel', '가족·배우자 관계 신호', '합과 충의 흐름으로 가족 개입, 거리 조절, 생활권 변화를 살핍니다.'),
    section('marriage-checklist', '현실 체크리스트', '결혼 전 반드시 말로 확인해야 할 조건입니다.', {
      bullets: [
        '주거 위치와 비용 부담 기준',
        '각자 가족과의 거리, 방문 빈도, 명절 기준',
        '월 고정비, 저축, 부채, 큰 지출의 결정권',
        '일이 바쁠 때 감정과 집안일을 어떻게 나눌지',
        '갈등이 생겼을 때 침묵, 회피, 폭발 중 어떤 패턴이 나오는지'
      ]
    })
  ];
}

function buildFocusedProductReport(report: SajuReportData): SajuReportData {
  if (report.kind === 'comprehensive') {
    return report;
  }

  const meta = FOCUSED_REPORT_META[report.kind];
  const currentYear = report.yearLuck[0];
  const sectionBuilders: Record<FocusedReportKind, (source: SajuReportData) => ReportSection[]> = {
    yearly: buildYearlyFocusedSections,
    love: buildLoveFocusedSections,
    reunion: buildReunionFocusedSections,
    marriage: buildMarriageFocusedSections,
    compatibility: buildCompatibilityFocusedSections
  };
  const sections = sectionBuilders[report.kind](report);
  ...
  return {
    ...
    badge: meta.badge,
    heroNote: `${report.customerName}님의 ${meta.coverLabel}입니다. 종합사주처럼 모든 영역을 길게 펼치기보다, 이 상품에서 고객이 가장 알고 싶어 하는 시기, 관계 장면, 선택 기준을 앞쪽 목차에 집중 배치했습니다.`,
    summary: {
      title: `${report.customerName}님의 ${meta.coverLabel} 핵심 요약`,
      analysis: [
        `${report.dayMaster} 일간과 ${report.currentDayun.name} 대운의 흐름을 기준으로, 이번 리포트는 ${meta.coverLabel.replace(' 리포트', '')}에 필요한 판단 기준만 추려서 읽습니다.`,
        currentYear
          ? `${currentYear.year}년의 핵심은 ${currentYear.headline}입니다. 이 흐름 안에서 ${currentYear.focus}을 먼저 잡고, ${currentYear.warning}은 무리하게 밀어붙이지 않는 편이 좋습니다.`
          : report.heroNote,
        `${bestMonthLabel(report)}에는 움직임을 열고, ${watchMonthLabel(report)}에는 감정적 확정보다 정리와 확인을 우선하면 리포트 활용도가 높아집니다.`
      ],
      advice: [
        '전체 운세를 넓게 보려 하기보다 이 상품의 주제와 직접 연결되는 선택만 먼저 정리하세요.',
        '좋은 시기에는 행동을 작게라도 열고, 약한 시기에는 결론보다 기준 정리를 우선하세요.',
        '상대, 돈, 일정, 체력 중 하나라도 무너지는 선택은 좋은 흐름에서도 오래 가기 어렵습니다.'
      ]
    },
    keyTakeaways: [
      productCard('핵심', `${meta.coverLabel.replace(' 리포트', '')}은 지금 ${report.currentDayun.focus} 흐름과 함께 봐야 정확합니다.`, 'good'),
      productCard('시기', `${bestMonthLabel(report)}에는 움직임을 열고 ${watchMonthLabel(report)}에는 조정과 확인을 우선하세요.`),
      productCard('기준', '감정, 기대, 불안보다 실제 행동과 반복되는 패턴을 기준으로 판단해야 합니다.'),
      productCard('주의', `${report.currentDayun.caution} 이 지점은 성급한 확정 전에 반드시 점검해야 합니다.`, 'warn')
    ],
    questionAnswers: report.questionAnswers.map((qa, index) => ({
      ...qa,
      title: `${meta.coverLabel.replace(' 리포트', '')} 질문 ${index + 1}: ${qa.title}`
    })),
    sections,
    actionPlan: {
      ...report.actionPlan,
      title: `${meta.coverLabel.replace(' 리포트', '')} 실행 체크`,
      priorities: [
        `1단계: ${sections[0]?.title || meta.coverLabel}을 먼저 읽고 지금 가장 큰 판단 기준을 하나만 정합니다.`,
        `2단계: ${bestMonthLabel(report)}에 할 수 있는 작은 행동을 정하고 바로 일정에 넣습니다.`,
        `3단계: ${watchMonthLabel(report)}에는 결론을 밀어붙이지 말고 돈, 시간, 관계, 체력 조건을 다시 확인합니다.`,
        '4단계: 한 달 뒤 실제 행동 결과를 기준으로 다음 선택을 조정합니다.'
      ],
      dos: [
        '좋은 흐름에는 작은 행동이라도 열기',
        '상대나 상황의 말보다 반복 행동 확인하기',
        '불안할수록 일정, 돈, 책임 범위를 글로 정리하기',
        '한 번의 반응보다 2주 이상 이어지는 패턴 보기'
      ],
      avoids: [
        '감정이 올라온 날 바로 결론 내리기',
        '상대의 말만 믿고 생활 조건을 확인하지 않기',
        '나쁜 시기를 실패로 단정하기',
        '좋은 시기라는 이유로 체력과 비용을 무리하게 쓰기'
      ]
    }
  };
}
...
function buildExpertSatisfactionReport(report: SajuReportData): SajuReportData {
  if (report.kind !== 'comprehensive') {
    return buildFocusedProductReport(report);
  }
      ...
      <div className={isYearlyShowcase ? 'premium-report-shell yearly-report-shell' : 'premium-report-shell'}>
        <article className={isYearlyShowcase ? 'premium-report-paper yearly-report-paper' : 'premium-report-paper'}>
          <section className={isYearlyShowcase ? 'premium-report-cover yearly-report-cover' : 'premium-report-cover'}>
            <h1>
              {report.customerName} {report.serviceId === 'general-signature' ? '종합사주 리포트' : report.title}
            </h1>
