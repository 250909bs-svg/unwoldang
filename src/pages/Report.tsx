...
} from '../products/past-life-goblin/share';
...
const PAST_LIFE_STORY_SECTION_IDS = new Set<string>(
...
function buildProductFocusedReport(report: SajuReportData): SajuReportData {
  ...
  if (report.serviceId === PAST_LIFE_PRODUCT_ID) {
    return ensurePastLifeGoblinReport(report);
  }
...
export default function Report() {
  ...
  const report = useMemo(() => {
    const finalizePastLife = (candidate: SajuReportData): SajuReportData => {
      const ensured = ensurePastLifeGoblinReport(candidate);

      if (ensured.serviceId !== PAST_LIFE_PRODUCT_ID || ensured.pastLifeProfile) {
        return ensured;
      }

      return {
        ...ensured,
        pastLifeProfile: buildPastLifeProfile(ensured, reportInput)
      };
    };

    // Paid/server reports are canonical artifacts that already passed the
    // immutable-fact guard. Complete paid prose remains untouched, while
    // legacy artifacts receive only a missing product contract or profile.
    if (reportData) {
      return finalizePastLife(reportData);
    }
  ...
  const isPastLifeShowcase = report.serviceId === PAST_LIFE_PRODUCT_ID;
  ...
  const handleShareReport = async () => {
    const shareData = isPastLifeShowcase
      ? createPastLifeProductShareData(window.location.origin)
