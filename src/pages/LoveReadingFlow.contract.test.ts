...
describe('MZ무당 팩폭 연애운 화면 순서 계약', () => {
  ...
  it('무료 미리보기는 원국·관계 장면을 풍성하게 공개하되 상세 미래 단서는 DOM에 넣지 않는다', () => {
    ...
    expect(previewSource).toContain('mz-love-story-art');
    expect(previewSource).toContain('mz-love-type-comparison');
    expect(previewSource).toContain('mz-love-portrait-vault');
    expect(previewSource).not.toContain('mz-love-vault-portrait');
    ...
    expect(previewSource).toContain('repeatedAttractionChapter?.factBomb');
    expect(previewSource).toContain('repeatedAttractionChapter?.realLifeScene');
    expect(previewSource).toContain("chapterById.get('attracted-partner')");
    expect(previewSource).toContain("chapterById.get('lasting-partner')");
    expect(previewSource).toContain("chapterById.get('relationship-status')");
    expect(previewSource).toContain('scene={isSingle ? futurePartnerScene : relationshipStatusChapter?.scene || roomScene}');
    expect(previewSource).toContain("nextLabel={isSingle ? '만남 환경 후보 보기' : '현재 관계 장면 보기'}");
    expect(previewSource).toContain('현재 관계 웹툰 본편 잠금 풀기');
    expect(previewSource).toContain('specificity.meeting.primaryContext');
    expect(previewSource).toContain('specificity.meeting.scene');
    expect(previewSource).toContain('viewModel.actionPlan.thirtyDays[0]');
    expect(previewSource).toContain('30일 계획 · 1주차 무료 공개');
    expect(previewSource).toContain('세부 외형·키 조합');
    expect(previewSource).toContain('직업 환경 Top 3·구체 장소');
    expect(previewSource).toContain('상대의 속마음이나 미래를 단정하지 않을게');
    expect(previewSource).toContain('특정 년·월에 사건이 생긴다고 단정하지 않고');
    expect(previewSource).toContain('조건별 관계 흐름 잠금 단계');
    expect(previewSource).toContain("label: '현재 기준', open: true");
    expect(previewSource).not.toContain('Array.from({ length: 12');
    expect(previewSource).not.toContain('<strong>NOW</strong>');
    expect(previewSource).toContain("matchMedia?.('(prefers-reduced-motion: reduce)')");
    expect(previewSource).toContain('tabIndex={-1}');
    expect(previewSource).not.toContain('getPremiumLoveAnswers');
    expect(previewSource).not.toContain('premiumAnswerById');
    ...
    expect(previewSource).not.toContain('specificity.height.label');
    expect(previewSource).not.toContain('specificity.face.primary');
    ...
    expect(previewSource).not.toContain('specificity.professions[0].fieldLabel');
