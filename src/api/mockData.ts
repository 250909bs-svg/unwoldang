...
export const serviceCatalog: ServiceDefinition[] = [
  ...
  },
  {
    ...
    label: '홍연아씨 재회 가능성',
    advisor: '청담 선생',
    subtitle: '두 사람의 명리 구조와 실제 이별·연락 행동을 분리해 읽는 재회 전략 리포트',
    teaser: '연락이 올까보다, 다시 만나도 같은 이유로 끝나지 않을 조건을 먼저 봅니다.',
    description:
      '절기 만세력과 정적 궁합, 이별 원인, 마지막 연락, 차단과 안전 신호를 서로 다른 근거로 분석합니다. 연락·답장·만남·재회·30일·90일 유지·재발 위험을 분리하고 근거와 반대 근거, 세 선택 비교, 메시지 검토와 멈춤 규칙을 함께 제공합니다.',
    price: '55,000원',
    accent: '#7d263d',
    theme: 'reunion',
    heroTag: 'REUNION LAB',
    badge: '안전 우선 재회 전략',
    spotlight: '다시 이어질지보다 다시 만나도 달라질 조건을 먼저 확인하는 리포트',
    bullets: ['14개 분리 지표', '근거·반대 근거', '세 선택 비교', '30일·90일 계획'],
    process: ['본인·상대 명리 계산', '이별·연락 사실 분리', 'SafetyGate 우선 판정', '지표·전략·QA 생성'],
    output: ['결론 우선 리포트', '최대 3개 절기 검토 창', '메시지·답장 트리', '재발 방지 30일·90일 계획']
  },
  {
    id: 'match-couple',
